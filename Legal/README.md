...
|---|---|---|
| GoogleFindMyTools | GPL-3.0 | `Proto/ProtoDecoders`, ports in `GoogleAuth`, `Nova`, `SpotClient`, `VaultUnlock`, `Crypto`, `ForeignTrackerCryptor`, `LocationDecrypt` |
| firebase-messaging (via GoogleFindMyTools) | MIT | `FcmRegister.swift`, `McsClient.swift` |
| gpsoauth | MIT | `GoogleAuth.swift` |
| encrypted-content-encoding (http_ece) | MIT | `HttpEce.swift` |
| micro-ecc | BSD-2-Clause | `Sources/uECC` |
| Chromium (checkin/MCS definitions) | BSD-3-Clause | `Proto/firebase`, `Sources/Generated/*.pb.swift` |
| SwiftProtobuf 1.38.1 | Apache-2.0 | SPM dependency; `SwiftProtobuf-LICENSE.txt` |
