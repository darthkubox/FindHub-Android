...
- Etykiety w kartach aplikacji używają `.labelStyle(.m3Icon)` (stałe pole ikony 24 pt), żeby tytuły stały w jednej linii mimo różnych szerokości symboli SF.
...
- Tryb demo (`DemoData`, `AppModel.startDemo/exitDemo`): przycisk na ekranie logowania, widoczny dla wszystkich (nie ukryty dla recenzenta — Apple 2.3.1). W demo żadnych zapytań do Google ani poleceń/skanowania Bluetooth; każda nowa funkcja sieciowa lub BLE musi mieć strażnika `isDemo` i działać na danych przykładowych. Zakończenie demo usuwa jego dziennik, nazwy, ikony i zdjęcia. Pilnuje tego `DemoModeTests`.
